...
/* K-平均法によるクラスタリングを行う */
{
    ...
    printf("元の階調画像のデータ量 = %d × %d × 8 = %d bit", x_size1, y_size1, original_data_size);
    i = K_number - 1;    j = 0;
    do{
        i = i / 2;   j ++;
    } while ( i > 0 );  /* j : 代表ベクトルの指定に必要なビット数 */
    coded_data_size = BLOCK_SIZE * BLOCK_SIZE * 8 * K_number
        /* 代表ベクトル */
        + 8 * (K_number / 4) * (K_number / 4);
			/* ルックアップテーブル */
    printf("圧縮データ（画像部分）のデータ量 = %d ", coded_data_size);
